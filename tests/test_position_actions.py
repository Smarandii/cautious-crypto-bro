...
def test_reduce_requires_percentage() -> None:
    import pytest

    with pytest.raises(
        ValueError,
        match="REDUCE requires close_pct",
    ):
        PositionActionIntent(
            source=source(),
            symbol="NEARUSDT",
...
def test_partial_reduce_cancels_ccb_entries_first() -> None:
    cancelled = False
    ...
    try:
        ...
        assert result.order_id == "reduce-1"
        assert result.submitted_quantity == Decimal("5")
        assert result.cancelled_entry_orders == 1
