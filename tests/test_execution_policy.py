...
def test_execution_policy_persists_only_risk(
    ...
    async def run() -> None:
        ...
        assert default.trading_capital_usdt is None
        ...
        assert loaded.trading_capital_usdt is None
        assert loaded.risk_per_trade_pct == Decimal("2")

        assert loaded.range_order_count == 3

        frozen = loaded.model_copy(
            update={
                "trading_capital_usdt": (Decimal("6800")),
            }
        )

        assert frozen.risk_budget_usdt == Decimal("136")

    asyncio.run(run())


def test_policy_requires_frozen_capital_for_budget() -> None:
    policy = ExecutionPolicy()

    try:
        _ = policy.risk_budget_usdt

    except ValueError as exc:
        assert "live-capital snapshot" in str(exc)

    else:
        raise AssertionError("Expected missing-capital failure")
