...
async def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Stop after OpenRouter extraction; "
            "do not build an execution plan."
        ),
    )
    ...
    if (
        args.intent_only
        and args.send_approval
    ):
        parser.error(
            "--send-approval requires "
            "an execution plan"
        )
    ...
    try:
        ...
        if not args.send_approval:
            ...
            print(
                "Use --send-approval to create "
                "a normal approval card."
            )
            return 0

        await (
            store.create_intent_with_plan(
                intent,
                plan,
            )
        )
